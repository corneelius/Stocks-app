...
public class Types {
	...
	public static String convertToPlainString(String string)
	{
		...
		while(true)
		{
			...
			ampersand = string.indexOf('&', ampersand);
			semicolon = string.indexOf(';', ampersand); //because code is replaced with char
		}
	}
	...
	public static Object[] convertIndexEntryArray(IndexEntry[] array)
	{
		//Object[0] = dates  Object[1] = bigDecimals
		String [] dates = new String[array.length];
		String[] bigDecimals = new String[array.length];
		for(int i=0; i<array.length; i++)
		{
			dates[i] = array[i].getDate().toString();
			bigDecimals[i] = array[i].getBigDecimal().toPlainString();
		}
		return new Object[] {dates, bigDecimals};
	}
	public static IndexEntry[] convertStringArrays(Object[] arrays)
	{
		//Object[0] = dates  Object[1] = bigDecimals
		IndexEntry[] ie = new IndexEntry[((Object[])arrays[0]).length];
		for(int i=0; i<((Object[])arrays[0]).length; i++)
		{
			ie[i] = new IndexEntry(new Date((String)((Object[])arrays[0])[i]), new BigDecimal((String)((Object[])arrays[1])[i]));
		}
		return ie;
	}
	...
	public static Date toDate(final String input)
	{
		...
		Log.e("nr.app", "date="+date2.toString());
		return date2;
	}
